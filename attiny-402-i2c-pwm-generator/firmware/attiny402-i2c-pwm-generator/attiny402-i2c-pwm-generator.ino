...
// Global variables
// ------------------------------------------------------------------------
volatile uint8_t device_registers[NUM_REG] = {0x00, 0x00, 0x00};
...
void setup() {
  ...
  // TODO: source these values from I2C registers
  setDutyCycle(0x7FFF, period);
  setFrequency(1750, &period);

  SERIAL_MSG("[PWM] init done (f: %d, d: 0x%X)", 1750, 0x7FFF);
}
